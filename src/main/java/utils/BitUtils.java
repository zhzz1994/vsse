...
public class BitUtils {
    ...
    public static byte[] setOne(byte[] bytes,int i){
        if(i >= bytes.length * 8){
            return bytes;
        }
        byte choosen = bytes[i / 8];
        int position = i % 8;
        byte newByte = setBitValue(choosen,position, (byte) 1);
        bytes[i / 8] = newByte;
        return bytes;
    }

    public static boolean isOne(byte[] bytes ,int i){
        if(i >= bytes.length * 8){
            return false;
        }
        byte choosen = bytes[i / 8];
        int position = i % 8;
        return checkBitValue(choosen,position);
    }
